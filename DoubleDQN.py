...
class environment():
    def __init__(self, origin_adr, destination_adr):
        ...
        self.charge_num = 0
        self.total_charge_num = 0  # 新增：全局充电累计变量
    ...
    def geocoding_api(self, address):  # 2 output: status, position
        ...
        geocode_url = geocode_api + urllib.parse.urlencode({'address': address}) + "&key=AIzaSyBxcVtnDTNCFN4LVKWX5UXRnt8spMdnLVA"
    ...
    def elevation_api(self, location):  # 2 output: elevation, resolution
        ...
        elevation_url = elevation_api + urllib.parse.urlencode({'locations': location}) + "&key=AIzaSyBxcVtnDTNCFN4LVKWX5UXRnt8spMdnLVA"
    ...
    def directions_api(self, origin, destination):  # 5 output: status, steps, bound
        ...
        directions_url = directions_api + urllib.parse.urlencode({'origin': origin}) + '&' + urllib.parse.urlencode(
            {'destination': destination}) + '&' + urllib.parse.urlencode({'units': 'metric'}) + "&key=AIzaSyBxcVtnDTNCFN4LVKWX5UXRnt8spMdnLVA"
    ...
    def origine_map_reward(self):  # to get the step_reward, chargenum, SOC, time which the route google provided
        self.step_reward = 0  # <--- 新增，确保属性已初始化
    ...
    def battery_charge(self):
        self.battery.charge(50000)
        self.battery.use(0, 0)
        self.charge_num = 0  # <- 新增，重置充电次数
        self.unreach_position_num = 0  # 建议也一并重置
        self.time = 0  # 建议也一并重置
        #self.battery = lithium_ion_battery(50000) #Wh
    ...
    def battery_condition(self):
        ...
        total_charge_numm = self.total_charge_num  # 新增
        return soc, charge_numm, total_charge_numm  # 新增
