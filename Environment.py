...
class Environment:
    def __init__(self, origin_adr, destination_adr):
        ...
        self.need_energy = EnergyCalculator()
        self.charge_num = 0
    ...
    def elevation_api(self, location):
        ...
        elevation_url = elevation_api + urllib.parse.urlencode(
            {'locations': location}) + "&key=AIzaSyBxcVtnDTNCFN4LVKWX5UXRnt8spMdnLVA"
    ...
    def directions_api(self, origin, destination):
        ...
        directions_url = directions_api + urllib.parse.urlencode({'origin': origin}) + '&' + urllib.parse.urlencode(
            {'destination': destination}) + '&' + urllib.parse.urlencode(
            {'units': 'metric'}) + "&key=AIzaSyBxcVtnDTNCFN4LVKWX5UXRnt8spMdnLVA"
    ...
    def make_map(self):
        ...
        self.Google_step = direction_steps
        while direction_status != 'OK':
            direction_status, direction_steps, self.map_bound = self.directions_api(origin_position,
        ...
        if (
            ...
            for i in range(len(leg_step)):
                ...
                for t in range(duration):
                    ...
                    need_charge = self.battery.use(duration=duration, power=power)
                    energy_consume += self.battery.energy_consumed  # ✅ 正确
